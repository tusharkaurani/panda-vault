import type { ChannelStatus, RebuildJob, SourceType } from "../types";
...
// A playlist is fetched rather than scanned, holds entries rather than
// files, and can never be `not_joined` — there is nothing to join.
const HINTS: Record<SourceType, Record<ChannelStatus, string>> = {
  telegram: {
  ...
  m3u: {
    ready: "Entries are indexed and searchable",
    ...
    empty: "Fetched, but no entries matched this playlist's extensions",
...
export default function StatusBadge({
  ...
  const unit = sourceType === "m3u" ? "entr" : "file";
  const live = job?.status === "running" ? job : undefined;
  const baseLabel = resolved === "empty" && sourceType === "m3u" ? "No entries" : style.label;
  const label = live
  ...
  return (
    <Tooltip
      label={
        live
          ? `${live.scanned.toLocaleString()} ${unit}${unit === "entr" ? "ies" : "s"} scanned so far`
